...
CREATE TRIGGER trigger_booking_added
...
CREATE TRIGGER trigger_booking_canceled
...
BEGIN 
	UPDATE FLIGHTS
	SET num_pass=num_pass + 1
	WHERE FNUMBER IN (SELECT FNUMBER FROM inserted) AND (SELECT STATUS FROM inserted) = 1
		
	UPDATE FLIGHTS
	SET num_pass=num_pass - 1
	WHERE FNUMBER IN (SELECT FNUMBER FROM inserted) AND (SELECT STATUS FROM inserted) = 0

	UPDATE AGENCIES
	SET num_book=num_book + 1
	WHERE NAME IN (SELECT AGENCY FROM inserted) AND (SELECT STATUS FROM inserted) = 1

	UPDATE AGENCIES
	SET num_book=num_book - 1
	WHERE NAME IN (SELECT AGENCY FROM inserted) AND (SELECT STATUS FROM inserted) = 0
END;
