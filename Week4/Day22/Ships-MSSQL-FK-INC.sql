...
ORDER BY CLASSES.COUNTRY ASC, SHIPS.NAME ASC;
...
SELECT ship1.SHIP AS ship
FROM OUTCOMES ship1
JOIN OUTCOMES ship2 ON ship1.SHIP = ship2.SHIP
JOIN BATTLES ON ship1.BATTLE = BATTLES.NAME
WHERE ship1.RESULT = 'damaged' 
AND (SELECT BATTLES.DATE FROM BATTLES WHERE BATTLES.NAME = ship2.BATTLE) > (SELECT BATTLES.DATE FROM BATTLES WHERE BATTLES.NAME = ship1.BATTLE);  